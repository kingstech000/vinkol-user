...
class _StorePaymentScreenState extends ConsumerState<StorePaymentScreen> {
  ...
  String?
      _paymentErrorMessage; // To display error message if payment fails to initiate
  ...
  void initState() {
    super.initState();
    _paymentReference =
        'STORE_ORDER_${_uuid.v4()}'; // Generate once per screen instance

    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref
          .read(appLoggerProvider)
          .d('[StorePaymentScreen] initState: addPostFrameCallback triggered.');
  ...
  Future<void> _initiatePaymentProcess() async {
    ...
    if (currentUser == null) {
      ref.read(appLoggerProvider).e(
          '[StorePaymentScreen] Error: User information missing for payment.');
      _showSnackbar('User information not found. Please log in again.');
      if (mounted)
        Navigator.of(context).pop(false); // Indicate failure back to CartScreen
      return;
    }

    final totalAmount =
        widget.arguments.totalProductAmount + widget.arguments.deliveryFee;
    ...
    try {
      ref.read(appLoggerProvider).d(
          '[StorePaymentScreen] Calling PaystackService.initializePayment...');
      final String? authUrl = await paystackService.initializePayment(
        ...
        callbackUrl:
            'https://your-app.com/paystack-callback', // **CRITICAL: Replace with your actual domain**
      );
      ...
      if (authUrl != null) {
        ...
        ref.read(appLoggerProvider).i(
            '[StorePaymentScreen] Received Paystack authorization URL: $_paystackAuthUrl');
        _initializeWebView(
        ...
        ref.read(appLoggerProvider).e(
            '[StorePaymentScreen] Failed to get Paystack authorization URL (service returned null).');
        setState(() {
      ...
      if (!mounted) return;
      ref.read(appLoggerProvider).e(
          '[StorePaymentScreen] Exception during payment initialization: $e\n$st',
          error: e,
          stackTrace: st);
      setState(() {
      ...
      if (mounted && _paystackAuthUrl == null) {
        // Only set loading to false if WebView didn't load
        setState(() {
  ...
  void _initializeWebView({
    ...
    ref.read(appLoggerProvider).d(
        '[StorePaymentScreen] _initializeWebView called with URL: $paystackAuthUrl');
      ...
      ..setNavigationDelegate(
        NavigationDelegate(
          ...
          onPageStarted: (String url) {
            ...
            ref
                .read(appLoggerProvider)
                .d('[StorePaymentScreen] WebView Page started loading: $url');
          },
          onPageFinished: (String url) {
            ...
            ref
                .read(appLoggerProvider)
                .d('[StorePaymentScreen] WebView Page finished loading: $url');
          },
          ...
          onNavigationRequest: (NavigationRequest request) async {
            ref.read(appLoggerProvider).d(
                '[StorePaymentScreen] WebView Navigating to: ${request.url}');
            ...
            if (request.url.contains(callbackDomain) &&
              ...
              ref.read(appLoggerProvider).d(
                  '[StorePaymentScreen] Detected callback URL with reference. Initiating backend verification...');
              final paystackService = ref.read(paystackServiceProvider);
              try {
                ...
                ref.read(appLoggerProvider).e(
                    '[StorePaymentScreen] Exception during payment verification: $e\n$st',
                    error: e,
                    stackTrace: st);
              ...
              return NavigationDecision
                  .prevent; // Prevent WebView from navigating to callback URL
            } else if (request.url.contains('paystack.com/close')) {
              ref
                  .read(appLoggerProvider)
                  .d('[StorePaymentScreen] Detected Paystack close URL.');
  ...
  void _showSnackbar(String message) {
    if (mounted) {
      ...
      ref
          .read(appLoggerProvider)
          .w('Attempted to show snackbar but widget was unmounted: $message');
    }
  }
  ...
  void _handlePaymentResult(bool success) {
    if (!mounted) {
      ref.read(appLoggerProvider).d(
          '[StorePaymentScreen] _handlePaymentResult called but widget is unmounted.');
      return;
    }

    if (success) {
      ref.read(appLoggerProvider).i(
          '[StorePaymentScreen] Payment successful. Proceeding to create order...');
      _showSnackbar('Payment successful! Creating your order...');
      _createStoreOrder(); // Now call the order creation logic
    } else {
      ref
          .read(appLoggerProvider)
          .w('[StorePaymentScreen] Payment failed or cancelled.');
      _showSnackbar('Payment failed or was cancelled.');
      Navigator.of(context)
          .pop(false); // Pop back to cart screen, indicating failure
    }
  }
  ...
  Future<void> _createStoreOrder() async {
    ...
    try {
      ...
      ref
          ...
          .clearCart(); // Assuming you have a clearCart method
      ...
      ref.read(appLoggerProvider).e(
          '[StorePaymentScreen] Failed to create store order after payment: $e\n$st',
          error: e,
          stackTrace: st);
      if (!mounted) return;
      _showSnackbar(
      ...
      Navigator.of(context)
          .pop(false); // Go back to cart, indicating order failure
    }
  }
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      onPressed: () {
                        ...
                        setState(() {
                          ...
                          _paystackAuthUrl =
                              null; // Reset auth URL to re-initiate
                          _isLoadingWebView = true;
                        });
                        _initiatePaymentProcess();
                      },
                    ...
                    OutlinedButton(
                      onPressed: () {
                        Navigator.of(context)
                            .pop(false); // Go back to cart screen
                      },
